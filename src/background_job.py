...
from src.db import ProcessedReceipt
...
async def insert_processed_receipt_to_db(processed_receipt_dict):
    async with SessionLocal() as session:
        async with session.begin():
            process_receipt=ProcessedReceipt(**processed_receipt_dict)
...
async def process_file(id,file_type,file_content):
    ...
    if file_type=='EMF':
        processed_receipt=await process_emf.process_receipt(id,file_content)
    elif file_type=='ESC/P':
        processed_receipt=await process_esc_p.process_receipt(id,file_content)
    elif file_type=='XPS':
        processed_receipt= await process_xps.process_receipt(id,file_content)
    elif file_type=='PDF':
        processed_receipt= await process_pdf.process_receipt(id,file_content)
    else:
        return
    if processed_receipt["image_link"] or processed_receipt["processed_text"]:
        prc_rec_id=await insert_processed_receipt_to_db(processed_receipt_dict=processed_receipt)
