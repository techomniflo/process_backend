...
async def process_receipt(id:int,file_content:bytes) -> Dict[str, Any]:
    """
    Processes a receipt by converting an PDF file to a PNG image and extracting text from it.
    ...
    async def extract_text():
        ...
        async with httpx.AsyncClient() as client:
            ...
            response =await client.post(url, files={"file": (f"{id}.pdf",file_content)},timeout=20.0)
            if response.status_code == 200:
